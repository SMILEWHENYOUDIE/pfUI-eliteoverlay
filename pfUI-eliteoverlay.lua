pfUI:RegisterModule("EliteOverlay", "vanilla:tbc", function ()
  ...
  if pfUI.gui.CreateGUIEntry then -- new pfUI
    pfUI.gui.CreateGUIEntry(T["Thirdparty"], T["Elite Overlay"], function()
      ...
      pfUI.gui.CreateConfig(pfUI.gui.UpdaterFunctions["target"], T["Nameplate position"], C.EliteOverlayNameplate, "position", "dropdown", pfUI.gui.dropdowns.EliteOverlayNameplate_positions)
	end)
  else -- old pfUI
  ...
  pfUI:UpdateConfig("EliteOverlayNameplate", nil, "position", "right")
  ...
  function pfUI.nameplates:OnDataChanged(plate)
	...
	local elite
		if string.find(levelText, "??B") then
			elite = "worldboss"
		elseif 
			string.find(levelText, "R+") then
			elite = "rareelite"
		elseif 
			string.find(levelText, "+") then
			elite = "elite"
		elseif 
			string.find(levelText, "R") == 1 then
			elite = "rare"
end
					
	plate.EliteOverlayNameplate = plate.EliteOverlayNameplate or plate.health:CreateTexture(nil, "OVERLAY")
  
	if C.EliteOverlayNameplate.position == "off" or not elite then
		plate.EliteOverlayNameplate:Hide()
		  ...
		  if elite == "worldboss" then
		    plate.EliteOverlayNameplate:SetVertexColor(.85,.15,.15,1)
		  ...
		  elseif elite == "elite" then
			...
			plate.EliteOverlayNameplate:Show()
		  elseif elite == "rare" then
			plate.EliteOverlayNameplate:SetVertexColor(.8,.8,.8,1)
			plate.EliteOverlayNameplate:SetTexture(texture)
			plate.EliteOverlayNameplate:Show()
		  else
			plate.EliteOverlayNameplate:Hide()
		...
		HookRefreshNameplate(this, plate)
  end
